...
\begin{itemize}
 ...
 \item \textbf{rlimit.as} -- Maximum memory to be allocated by a single request. Defaults to 2 GB.
 \item \textbf{rlimit.fsize} -- Maximum size of files that a request can create. Defaults to 100MB.
