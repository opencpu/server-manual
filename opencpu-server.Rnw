...
The \OpenCPU cloud server version 2.0 runs on \textbf{Ubuntu 16.04 (Xenial) or higher}. The \OpenCPU system consists of a number of standard Ubuntu installation packages. These are:
