...
The \OpenCPU cloud server version 1.5 runs on \textbf{Ubuntu 14.04 (trusty) or higher}. The \OpenCPU system consists of a number of standard Ubuntu installation packages. These are:
...
\begin{codeblock}
sudo add-apt-repository ppa:opencpu/opencpu-1.5 -y
sudo apt-get update
\end{codeblock}
