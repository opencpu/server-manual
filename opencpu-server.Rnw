...
The \OpenCPU cloud server version 2.1 runs on \textbf{Ubuntu 18.04 or 16.04}. The \OpenCPU system consists of a number of standard Ubuntu installation packages. These are:
