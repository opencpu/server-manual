...
A complete overview and demonstration of the OpenCPU api is available on the website: \href{http://www.opencpu.org}{www.opencpu.org}. Below a few basic examples to illustrate basic functionality and verify that things are working. Also note that OpenCPU ships with a little testing page that can be used to do http requests. The testing page can be found at \url{http://your.server.com/ocpu/test}. 
...
\begin{verbatim}
  curl -X POST http://your.server.com/ocpu/library/knitr/examples/knitr-minimal.Rnw
  curl -X POST http://your.server.com/ocpu/library/knitr/examples/knitr-minimal.Rmd
  curl -X POST http://your.server.com/ocpu/library/knitr/doc/knitr-intro.R
  curl -X POST http://your.server.com/ocpu/library/brew/example1.brew
\end{verbatim}
...
\noindent When a non-root user installs a package in RStudio, the package will not be installed in the global library, but in the user's personal package library. User's personal package libraries are available through the API at the \texttt{/user/:name} API, e.g: \\

\indent \url{http://your.server.com/ocpu/user/jeroen/library} \\

\noindent Note that the package will only be available through the OpenCPU if the user has set the file mode permissions (\texttt{chmod}) to public readable, which is the default on Ubuntu. 
