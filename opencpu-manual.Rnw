...
\noindent The application can then be opened in a browser. If the application was installed on an OpenCPU cloud server, it would now be available at \href{http://localhost/ocpu/library/gitstats/www}{http://localhost/ocpu/library/gitstats/www}. \\
