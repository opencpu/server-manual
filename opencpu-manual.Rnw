...
\section{Testing OpenCPU Installation}
...
\noindent Now try calling some functions and scripts using \texttt{HTTP POST}. To do a \texttt{POST} request, we cannot simply open the URL in the browser. Either use the testing webapp, or use \texttt{curl} with the \texttt{-X} or \texttt{-d} flags. For example:

\begin{verbatim}
  curl -X POST http://localhost/ocpu/library/utils/R/sessionInfo
  curl http://localhost/ocpu/library/stats/R/rnorm -d n=10\&mean=100
\end{verbatim}
In OpenCPU, a successful \texttt{POST} requests usually returns a \texttt{HTTP 201} status, and the response body contains the location of the output data. For example, it might look like this:
...
\noindent The following endpoints represent external repositories. These are only available if they are enabled in the server configuration and if the server has internet access. The API is identical to the one above, with the difference that the \texttt{/library/} has been replaced with some external repository.

\begin{verbatim}
http://localhost/cran/plyr
http://localhost/cran/plyr/R
http://localhost/cran/plyr/R/ldply
http://localhost/cran/plyr/man
http://localhost/cran/plyr/man/ldply
http://localhost/cran/plyr/man/ldply/html
\end{verbatim}
